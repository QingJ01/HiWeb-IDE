...
public class HopWebWebView extends WebView {
	...
	public WebChromeClient wcc = new WebChromeClient() {
		...
		public boolean onJsConfirm(WebView view, String url, String message, final JsResult result) {
			...
			Ly.setPadding(Do.dp2px(getContext(), 10), Do.dp2px(getContext(), 10), Do.dp2px(getContext(), 10),
	...
	public WebViewClient wvc = new WebViewClient() {
		...
		public void onReceivedHttpError(WebView view, WebResourceRequest request, WebResourceResponse errorResponse) {
			...
			if (request.isForMainFrame()) {
				...
				// +"<br><div
				// style='font-size:x-large;'>"+getContext().getString(R.string.cant_open_page)+"</div><br><span
				// style='font-size:small;'>"+getContext().getString(R.string.description)+errorResponse.getReasonPhrase()+"</span></div></body>";
				// view.loadUrl("javascript:document.innerHTML=\"" + data + "\"",null);
		...
		public void onReceivedError(WebView view, WebResourceRequest request, WebResourceError error) {
			...
			if (request.isForMainFrame()) {
				...
				// +"<br><div
				// style='font-size:x-large;'>"+getContext().getString(R.string.cant_open_page)+"</div><br><span
				// style='font-size:small;'>"+getContext().getString(R.string.description)+error.getDescription()+"</span></div></body>";
				// view.loadUrl("javascript:document.innerHTML=\"" + data + "\"",null);
		...
		public boolean shouldOverrideUrlLoading(WebView view, String newurl) {
			try {
				...
				if (!newurl.startsWith("http://") && !newurl.startsWith("https://") && !newurl.startsWith("file://")
						&& !newurl.startsWith("ftp://") && !newurl.startsWith("ftps://")
						&& !newurl.startsWith("sftp://") && !newurl.startsWith("javascript:")
					...
					try {
						...
						// 防止没有安装的情况
						e.printStackTrace();
						wcc.onConsoleMessage(new ConsoleMessage(getContext().getString(R.string.start_intent_debug),
								newurl, 0, ConsoleMessage.MessageLevel.ERROR));
					}
					return true;
				}
			} catch (Exception e) {
