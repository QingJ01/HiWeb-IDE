...
public class PagePreviewer {
	...
	public static void previewNowFile(boolean mustLocal) {
		...
		if (Vers.i.FileType == 7) {
			if (Vers.i.nowProjectServerType != 2 || Vers.i.ProjectDir == null
					|| !nowFile.getPath().startsWith(Vers.i.ProjectDir.getPath() + "/")) {
				main.toast(R.string.preview_other_webpage_err);
				return;
			}
		}

		if (!mustLocal && Vers.i.ProjectDir != null && nowFile.getPath().startsWith(Vers.i.ProjectDir.getPath() + "/")
			...
			main.save(new Runnable() {
				...
				@Override
				public void run() {
					main.PreviewHwwv.loadUrl("http://" + host + ":" + Vers.i.nowWebsitePort
							+ nowFile.getPath().replace(Vers.i.ProjectDir.getPath(), ""));
				}
			});
		...
		if (Vers.i.FileType == 0) {
			...
			main.PreviewHwwv.wvc.onPageStarted(main.PreviewHwwv, "file://" + Do.URLEncode(Vers.i.OpenFile.getPath()),
					null);
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						final String output = Do.runCommand(
								new String[] { "/system/bin/sh", "-c", "export LD_LIBRARY_PATH=\"" + libDir.getPath()
										+ "\" && \"" + phpFile.getPath() + "\" -f \"" + nowFile.getPath() + "\"" });
						main.runOnUiThread(new Runnable() {

							@Override
							public void run() {
								main.PreviewHwwv.wvc.onPageFinished(main.PreviewHwwv,
										"file://" + Do.URLEncode(Vers.i.OpenFile.getPath()));
								if (SettingsClass.isAllowAbsolute) {
									main.save(new Runnable() {

										@Override
										public void run() {
											main.PreviewHwwv.loadDataWithBaseURL(baseURL, output, "text/html", "utf-8",
													null);
										}
									});
								} else {
						...
						Do.showWaitAndRunInThread(true, new Runnable() {
							...
							public void run() {
								...
								try {
									...
									Do.downloadNet(
											Vers.i.serverHost + "/server/hiweb/php/php_" + Vers.phpVersion + ".zip",
											compressedPHPFile.getPath());
									...
									main.runOnUiThread(new Runnable() {
										...
										public void run() {
											Do.finishWaiting();
											new Dl(main, main.getString(R.string.done),
													main.getString(R.string.php_download_done),
									...
									main.runOnUiThread(new Runnable() {
										...
										public void run() {
											...
											dlDone.builder.setNeutralButton(R.string.solve_proj,
	...
	public static void previewWithSettings() {
		...
		cl.build(false, -1, null, R.string.review_origin,
				main.getString(R.string.review_from_locale) + "|" + main.getString(R.string.review_from_website_server)
						+ "|" + main.getString(R.string.review_from_custom));
			...
			public void onClick(DialogInterface p1, int p2) {
				...
				switch (cl.Acs.getSelectedItemPosition()) {
					...
					case 1:
						...
						break;
					case 2:
						cusStringArr = new String[] { baseUrlTl.getText("$"), mimeTypeTl.getText("$"),
								encodingTl.getText("$"), historyTl.getText("$") };
						...
						if (SettingsClass.isAllowAbsolute) {
							main.save(new Runnable() {
								...
								@Override
								public void run() {
									main.PreviewHwwv.loadDataWithBaseURL(
											baseUrlTl.getText("$") == null ? "" : baseUrlTl.getText("$"), HTMLText,
											mimeTypeTl.getText("$") == null ? "" : mimeTypeTl.getText("$"),
											encodingTl.getText("$") == null ? "" : encodingTl.getText("$"),
											historyTl.getText("$") == null ? "" : historyTl.getText("$"));
								}
							});
						} else {
							main.PreviewHwwv.loadDataWithBaseURL(
