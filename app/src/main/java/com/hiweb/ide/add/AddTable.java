...
public class AddTable {
    ...
    class AnonymousClass100000000 implements OnClickListener {
        ...
        public void onClick(DialogInterface dialogInterface, int i) {
            ...
            int i2 = text.equals("None") ? AddTable.HEADER_NONE : text.equals("Top") ? AddTable.HEADER_TOP : text.equals("Left") ? AddTable.HEADER_LEFT : -1;
    ...
    public AddTable(Context ctx, Editor Eo)
    {
        ...
        choiceLayout.build(true,5,"Frame", R.string.add_table_frame, "void|above|below|hsides|vsides|lhs|rhs|box|border");
        this.Ly.addView(choiceLayout);
        choiceLayout = new ChoiceLayout(ctx);
        ...
        this.Ly.addView(Tl);
		choiceLayout = new ChoiceLayout(ctx);
        choiceLayout.build(false,8,null, R.string.is_show_table_nums, ctx.getString(R.string.enable)+"|"+ctx.getString(R.string.disable));
        ...
        dl.builder.setPositiveButton(R.string.insert, new AnonymousClass100000000(this,(ChoiceLayout) Ly.findViewById(2), ctx));
        dl.show();
    }
        ...
        try {
            ...
            switch (i) {
                case HEADER_NONE /*0*/:
                    for (i2 = HEADER_NONE; i2 < parseInt; i2 += HEADER_LEFT) {
                        ...
                        for (i3 = HEADER_NONE; i3 < parseInt2; i3 += HEADER_LEFT) {
                            stringBuilder.append(new StringBuffer().append(new StringBuffer().append(new StringBuffer().append(new StringBuffer().append(new StringBuffer().append(str3).append(str3).toString()).append("<td>").toString()))));
							if(isShowNums)
                ...
                case HEADER_LEFT /*1*/:
                    for (i2 = HEADER_NONE; i2 < parseInt; i2 += HEADER_LEFT) {
                        stringBuilder.append(new StringBuffer().append(str3).append("<tr>\n").toString());
                        stringBuilder.append(new StringBuffer().append(new StringBuffer().append(new StringBuffer().append(new StringBuffer().append(new StringBuffer().append(str3).append(str3).toString()).append("<th>").toString()))));
						if(isShowNums)
