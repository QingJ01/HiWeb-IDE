...
public class UserLoginer {
    ...
    private static boolean saveInf(String user) {
        if (user != null
                && (!user.trim().equals("") && (user.trim().indexOf("\\") == -1 && user.trim().indexOf("\"") == -1
                        && user.trim().indexOf("/") == -1 && user.trim().indexOf("&") == -1 && user.length() <= 12))) {
    ...
    public static void showUserInf() {
        ...
        ActvHello.setText(MainActivity.main.getString(R.string.hello) + Vers.i.userName
                + MainActivity.main.getString(R.string.control_username));
                    ...
                    dl.builder.setItems(items, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface p1, int p2) {
                            switch (p2) {
                                case 0:
                                    try {
                                        Do.copyDir(MainActivity.main.getFilesDir().getPath(),
                                                Vers.i.FVenter.getPath() + "/files/");
                                    } catch (Exception e) {
                                        Do.showErrDialog(MainActivity.main, e);
                                    }
                                    break;
