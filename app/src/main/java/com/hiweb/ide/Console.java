...
public class Console extends LinearLayout
{
    ...
    public Console(android.content.Context context, android.util.AttributeSet attrs, int defStyleAttr, int defStyleRes)
	{
	...
	*/
	...
	public WebChromeClient wcc=new WebChromeClient()
	{
		...
		public boolean onConsoleMessage(ConsoleMessage consoleMessage)
		{
			...
			if (consoleMessage.messageLevel()==ConsoleMessage.MessageLevel.DEBUG)
			{
				...
				text.setSpan(new ForegroundColorSpan(Color.GRAY), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
			...
			else if(consoleMessage.messageLevel()==ConsoleMessage.MessageLevel.LOG)
			{
				...
				text=new SpannableString(title+consoleMessage.message());		
				text.setSpan(new ForegroundColorSpan(ContextCompat.getColor(MainActivity.main,R.color.opposition)), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
				text.setSpan(new StyleSpan(Typeface.BOLD), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
			}
			else if(consoleMessage.messageLevel()==ConsoleMessage.MessageLevel.TIP)
			{
				...
				text=new SpannableString(title+consoleMessage.message());		
				text.setSpan(new ForegroundColorSpan(Color.BLUE), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
				text.setSpan(new StyleSpan(Typeface.BOLD), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
			}
			else if(consoleMessage.messageLevel()==ConsoleMessage.MessageLevel.WARNING)
			{
				...
				text=new SpannableString(title+consoleMessage.message());		
				text.setSpan(new ForegroundColorSpan(Color.parseColor("#FF8000")), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
				text.setSpan(new StyleSpan(Typeface.BOLD), 0, title.length(), Spannable.SPAN_INCLUSIVE_INCLUSIVE);
			}
			String source="";
			if(consoleMessage.sourceId().equals("file://"+new File(getContext().getFilesDir(),"temp_javascript").getPath()))
			{
	...
	public void updateList()
	{
		...
		{
			lv.setAdapter(new ArrayAdapter(getContext(),android.R.layout.simple_list_item_1,new String[]{getContext().getString(R.string.no_items)}));
		...
		lv.setOnItemClickListener(new AdapterView.OnItemClickListener(){
				...
				public void onItemClick(AdapterView<?> p1, View p2, int p3, long p4)
				{
					...
					tv.setPadding(Do.dp2px(MainActivity.main,10),Do.dp2px(MainActivity.main,10),Do.dp2px(MainActivity.main,10),Do.dp2px(MainActivity.main,10));
