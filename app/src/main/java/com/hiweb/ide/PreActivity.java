...
public class PreActivity extends BaseActivity {
	...
	private void init() {
		try {
			...
			Vers.i.FProjects = new File(Environment.getExternalStorageDirectory().getPath() + "/Venter/EasyWeb",
					"Projects");
	...
	public void readSetting() {
		try {
			...
			JsonArray JaChars = isContainAttribution(JoAll, "chars") ? JoAll.get("chars").getAsJsonArray()
					: new JsonArray();
			JsonArray JaNames = isContainAttribution(JoAll, "names") ? JoAll.get("names").getAsJsonArray()
					: new JsonArray();
			int ITheme = isContainAttribution(JoAll, "theme") ? JoAll.get("theme").getAsInt() : -1;
			boolean vBISDarktheme = isContainAttribution(JoAll, "isDarkTheme") ? JoAll.get("isDarkTheme").getAsBoolean()
					: false;
			int vIBackup = isContainAttribution(JoAll, "backup_frequency") ? JoAll.get("backup_frequency").getAsInt()
					: 60;
			File vFQsWebsite = isContainAttribution(JoAll, "custom_quickstart")
					? new File(JoAll.get("custom_quickstart").getAsString())
					: null;
			boolean isDialogRound = isContainAttribution(JoAll, "isDialogRound")
					? JoAll.get("isDialogRound").getAsBoolean()
					: false;
			boolean isAutoPreviewFull = isContainAttribution(JoAll, "isAutoPreviewFull")
					? JoAll.get("isAutoPreviewFull").getAsBoolean()
					: false;
			boolean isAllowAbsolute = isContainAttribution(JoAll, "isAllowAbsolutePath")
					? JoAll.get("isAllowAbsolutePath").getAsBoolean()
					: false;
			boolean isHideHopWeb = isContainAttribution(JoAll, "isHideHopWeb")
					? JoAll.get("isHideHopWeb").getAsBoolean()
					: false;
			boolean isShowWebViewAlert = isContainAttribution(JoAll, "isShowWebViewAlert")
					? JoAll.get("isShowWebViewAlert").getAsBoolean()
					: true;
