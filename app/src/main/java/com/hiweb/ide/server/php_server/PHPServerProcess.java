	...
	public int onStartCommand(Intent intent, int flags, int startId)
	{
		if(intent.getBooleanExtra("startPHPWebsite",false))
		{
			...
			try
			{
				...
				Do.write(port+"//////////"+projectDir.getPath()+"//////////"+android.os.Process.myPid(),profitFile);
				...
				new Thread(() -> {
					...
					final String msg;
					try {
						msg = Do.runCommand(new String[]{"/system/bin/sh","-c","export LD_LIBRARY_PATH=\""+lib.getPath()+"\" && \""+php.getPath()+"\" -S 0.0.0.0:" + port+" -t \""+projectDir.getPath()+"\""});
