...
public class UpdateManager {
	...
	public static void check(final boolean isShowToast, final BaseActivity activity) {
		...
		Runnable runnable = new Runnable() {
			...
			@Override
			public void run() {
				vers = Do.testGetHtml(Vers.i.serverHost + "server/hiweb/update/lastest_version.txt", 6);

				Vers.i.easyAppNewestVersion = Do.testGetHtml(Vers.i.serverHost + "server/hiweb/easyapp/version.txt",
						6);
					...
					try {
						if (vers != null && vers.indexOf("<andnext>") != -1 && vers.split("<andnext>").length == 3) {
							...
							if (newVersionCode > nowVersionCode) {
								...
								dl.builder.setPositiveButton(R.string.main_update_btn,
