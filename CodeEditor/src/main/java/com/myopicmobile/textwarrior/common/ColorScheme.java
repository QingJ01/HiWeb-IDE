...
public abstract class ColorScheme
{
	...
	private HashMap<Colorable, Integer> generateDefaultColors()
	{
		...
		colors.put(Colorable.SELECTION_FOREGROUND, 0xFFE0E0E0);//选择文本的前景色
		colors.put(Colorable.SELECTION_BACKGROUND, 0xFF2ABFFF);//选择文本的背景色
		...
		colors.put(Colorable.LINE_HIGHLIGHT, 0x10000000);

		colors.put(Colorable.NON_PRINTING_GLYPH, 0xFFBDBDBD);//行号
		colors.put(Colorable.COMMENT, OLIVE_GREEN); //注释
		colors.put(Colorable.KEYWORD, 0xFF0096FF); //关键字
		colors.put(Colorable.NAME, GREY); // Eclipse default color
		colors.put(Colorable.NUMBER, 0xFFE91E63); // 数字
		colors.put(Colorable.STRING, DARK_RED); //字符串
		colors.put(Colorable.SECONDARY, 0xFF03A9F4);//宏定义
		return colors;
	}
