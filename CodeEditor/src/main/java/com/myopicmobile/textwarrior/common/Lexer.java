...
public class Lexer
{
	...
	public void tokenize(DocumentProvider hDoc,int mode)
	{
		...
		{
			_workerThread = new LexThread(this,mode);
			_workerThread.start();
		}
		else
		{
	...
	private class LexThread extends Thread
	{
		...
		int lang=-1;
		public LexThread(Lexer p,int mode)
		{
			_lexManager = p;
			_abort = new Flag();
			lang=mode;
		...
		public void tokenize(int lang){
			...
			int idx=0;
				while (cType!=CType.EOF){
					try {
						cType=cLexer.yylex(lang);
						
						String name = cLexer.yytext();
						if(language.isOperator(name.charAt(0))&&cType!=cType.STRING&&cType!=cType.STRING_LITERAL&&cType!=cType.CHARACTER_LITERAL&&cType!=cType.COMMENT)
						{
							tokens.add(new Pair(idx, OPERATOR));
						}
						else if(language.isKeyword(name)&&cType!=cType.STRING&&cType!=cType.STRING_LITERAL&&cType!=cType.CHARACTER_LITERAL&&cType!=cType.COMMENT)
						{
							tokens.add(new Pair(idx, KEYWORD));
						}
						else if(language.isName(name)&&cType!=cType.STRING&&cType!=cType.STRING_LITERAL&&cType!=cType.CHARACTER_LITERAL&&cType!=cType.COMMENT)
						{
